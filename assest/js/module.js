...
export const getDate = (dateUnix, timezone, includeWeekday = false) => {
    const date = new Date((dateUnix + timezone) * 1000);
    const weekDayName = weekDayNames[date.getUTCDay()];
    const monthName = monthNames[date.getUTCMonth()];
    const day = date.getUTCDate();
    const formattedDate = includeWeekday
        ? `${weekDayName}, ${day} ${monthName}`
        : `${day} ${monthName}`;
...
export const getHours = (timeUnix, timezone) => {
    // Convert timezone (seconds) to milliseconds and adjust the timestamp
    const date = new Date((timeUnix + timezone) * 1000);

    const hours = date.getHours(); // Use getHours() for local time
